...
问题：无法保证节点在断连到重连期间，分区内的拓扑结构没有任何变化，可能又有许多别的新节点加入，也可能之前的节点退出了。

解决：网络中的 Booter 节点不仅负责进行区块排序服务，同时还担任 P2P 网络构建过程中“引导节点”的工作，其他节点在加入到区块链网络之前都必须先与 Booter 节点建立连接（Booter 节点的地址是固定的），通过 Booter 间接的与其他节点进行连接，因此可以**让 Booter 节点维护区块链网络的最新拓扑结构**。
