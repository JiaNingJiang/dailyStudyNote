...
当创建 `String` 类型的对象时，**虚拟机会在常量池中查找有没有已经存在的值和要创建的值相同的对象，如果有就把它赋给当前引用**。如果**没有就在常量池中重新创建一个 `String` 对象**。
...
![hashCode() 方法](https://oss.javaguide.cn/github/javaguide/java/basis/java-hashcode-method.png)hashCode() 方法
...
这是因为**两个对象的`hashCode` 值相等并不代表两个对象就相等**。
