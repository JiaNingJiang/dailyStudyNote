...
除了 **`BootstrapClassLoader` 是 JVM 自身的一部分**之外，其他所有的类加载器都是在 JVM 外部实现的，并且全都继承自 `ClassLoader`抽象类。这样做的好处是用户可以自定义类加载器，以便让应用程序自己决定如何去获取所需的类。
...
另外，**类加载器之间的父子关系一般不是以继承的关系来实现的**，而是**通常使用组合关系来复用父加载器的代码**。
