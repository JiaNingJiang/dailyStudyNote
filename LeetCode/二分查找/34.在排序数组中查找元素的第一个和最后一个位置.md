...
func searchRange(nums []int, target int) []int {
    ...
    // 先找左边界
    for {
        if left > right {  // 退出时，left最小是0，那么right最小就是-1
            break
        }
        mid = left + (right - left) / 2
        if target == nums[mid] {   // 缩小到左半区域（因为是在找左边界）
            right = mid - 1
    ...
    // right 的取值范围是: [-1, len(nums) - 1]    mid = right + 1, nums[mid]是最后一个等于target的数字，mid的范围是：[0, len(nums)]
    // right == -1 时，mid == 0   分为两种情况: 1. nums[mid] == target（退出之前正好查询到 target）  2.nums[mid] != target（退出前并没有查询到 target ）
    // right == len(nums) - 1 时, mid == len(nums)，此时访问 nums[mid] 是越界的 
    if right == len(nums) - 1 || nums[right + 1] != target {  
        mostLeft = -1  // 这两种情况下，说明 target 并不存在于 nums 中
    } else {
    ...
    for {
        ...
        mid = left + (right - left) / 2
        if target == nums[mid] {   // 缩小到右半区域（因为是找右边界）
            left = mid + 1
