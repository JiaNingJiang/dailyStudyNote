...
func Mul(a, b int32) int32 {
	var res int32 = 0
	var multiplicand int32 = a // 乘数
	var bitProduct int32 = b   // 乘数与被乘数某一位的bit乘积(要么为0，要么为被乘数b<<n)

	// 遍历被乘数的每一bit位，与乘数相乘
	for {
		if multiplicand == 0 { // 完成乘数的bit遍历
			break
		}
		if (multiplicand & 1) != 0 { // 获取乘数当前的最后一位，如果是1的话加上当前位的bit乘积
...
func Div(dividend, divisor int32) int32 {
	...
	if isNeg(dividend) == 1 { // 保证参与除法运算的被除数和除数都是正整数
