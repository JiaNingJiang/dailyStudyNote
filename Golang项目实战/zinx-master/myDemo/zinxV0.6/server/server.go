...
func DoConnectionBegin(conn ziface.IConnection) {
	...
	fmt.Println("[server] Register some proprerty for client connection")
	//为连接注册一些连接属性
	conn.AddProperty("Name", "Player1")
	conn.AddProperty("HP", 1000)
	conn.AddProperty("MP", 500)
}
...
func DoConnectionLost(conn ziface.IConnection) {
	...
	if property, err := conn.GetProperty("HP"); err == nil {
		fmt.Println("HP :", property)
	}
	if property, err := conn.GetProperty("MP"); err == nil {
		fmt.Println("MP :", property)
	}
}
