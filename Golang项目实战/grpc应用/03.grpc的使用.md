...
在下述的文件中，`service`目录和其下的两个go源文件即是由上述命令生成的。其中：`demo.pb.go`是message的生成go文件；`demo_grpc.pb.go`是service生成的go文件。

<img src="03.grpc的使用.assets/image-20230223210945150.png" alt="image-20230223210945150" style="zoom:67%;" />

**`demo_grpc_pb.go`提供了rpc服务接口如下：**

![image-20230223205623781](03.grpc的使用.assets/image-20230223205623781.png)
...
- 第10行我们new了一个service.ServiceProduct类的对象ServiceObject，它实现了**ProductServiceServer接口**(需要我们手动进行实现)，这个接口就是我们在 grpc_product.proto给出的  ProductService 的service 通过 protoc-gen-go-grpc.exe 工具生成的。
...
func (sp *ServiceProduct) Service(context context.Context, req *ProductRequest) (*ProductResponse, error) {
	result := sp.GetResult(req.ReqId)   // 方法内部实现需要由用户自行实现
...
在 Server，主要留意 `stream.Send` 方法。它看上去能发送 N 次？有没有**大小限制**？
...
func (s *StreamService) Route(stream pb.StreamService_RouteServer) error {
    n := 0
    for {
        err := stream.Send(&pb.StreamResponse{   // 发送一次
            Pt: &pb.StreamPoint{
